...
let handler = async (m, { conn, usedPrefix }) => {
  ...
  try {
    ...
    if (!levelling.canLevelUp(user.level, user.exp, global.multiplier)) {
      ...
      rank.build()
        .then(async data => {
          await conn.sendButtonImg(m.chat, `Level *${user.level} (${user.exp - min}/${xp})*\nKurang *${max - user.exp}* lagi!`.trim(), data, author, 'Auto Level Up', `${usedPrefix}on autolevelup`, { height: 282, width: 934 })
        })
    }
    ...
    if (before !== user.level) {
      ...
      rank.build()
        .then(async data => {
          await conn.sendButtonImg(m.chat, `_*Level Up!*_\n_${before}_ -> _${user.level}_`.trim(), data, author, 'Auto Level Up', `${usedPrefix}on autolevelup`, { height: 282, width: 934 })
        })
    }
  }
}
